...
- **Invoke `vc-x1-dev`, never `vc-x1`** (recorded 2026-08-02): the bare `vc-x1` on `$PATH` is
  the stable 0.71.0 main-branch install; the dev artifact installs beside it as `vc-x1-dev`
  and is the dogfood target for every invocation this repo's work makes (push, squash-push,
  validate-*, fix-*).
...
- 2026-08-02: the 0.78.0-9 push ran on the stable 0.71.0 binary, not the dev build
  ...
  - fixed by the "Invoke `vc-x1-dev`, never `vc-x1`" convention above; template finding: a
    dev-artifact repo needs its custom.md to name the dogfood binary explicitly
