...
49. **OSC 8 hyperlinks in `config` TTY output** (wink, 2026-08-09). When stdout is a TTY,
...
50. **init distributes vc-config.md; reference-base points at the member** (wink + bot,
...
51. **Reference defs: go file-relative, with anchors** (wink + bot, 2026-08-10). The house
...
52. **vc-config.md per-key worked examples** (2026-08-11). The per-key sections mostly restate
...
53. **Config provenance names the schema, not just the binary** (iiac-perf + bot, 2026-08-12).
