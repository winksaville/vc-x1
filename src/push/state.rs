...
//! stage) so the stage bodies review in a file free of the state
//! plumbing:
//!
...
//!
//! The stateless-push stage retires most of this: once the resume
//! point is derived from repo reality, `PushState` persistence and
//! its escape helpers go, leaving `Stage` as the sequence's name.
...
/// are required. See `push.rs`'s module docstring for which dev step
