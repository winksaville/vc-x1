...
**Refactor stage: hygiene riders.** Terminology stragglers
from the 0.69.0-4 work/bot sweep plus the single-field
`options_flags` leaves that still name their field after the
flag — sweeps that churn the lines later stages rewrite, done
early so those stages diff cleanly; see
[the stage](notes/refactor-20260716.md#stage-hygiene-riders).
Decisions (2026-07-22): rename all straggler identifiers;
